...
export async function getLeadInsights(lead: Lead) {
  try {
...
export async function getDashboardSummary(kpis: KPIData, leadsCount: number) {
  try {
    const response = await ai.models.generateContent({
      model: "gemini-3-flash-preview",
...
export async function generateNotificationTemplate(contexto: string, canal: string) {
  try {
    const response = await ai.models.generateContent({
      model: "gemini-3-flash-preview",
